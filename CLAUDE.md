...
- **Tenancy invariant**: every user has a personal organization whose `organizations.id == users.id`. Billing tables (`transactions`, `orders`, `subscriptions`, `budgets`) always point `org_id` at a real `organizations.id`, enforced by FKs after migration 000021. When a handler stores `OrgID: userID` for a personal-workspace user, that's still valid — the row exists in `organizations` with the same UUID.
