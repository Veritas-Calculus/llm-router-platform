...
export default function WebhooksPage() {
  ...
  const [updateWebhook] = useMutation(UPDATE_WEBHOOK_ENDPOINT, {
    onCompleted: () => {
      toast.success(t('webhooks.updated_success'));
      refetch();
      setIsModalOpen(false);
  ...
  const [deleteWebhook] = useMutation(DELETE_WEBHOOK_ENDPOINT, {
    onCompleted: () => {
      toast.success(t('webhooks.deleted_success'));
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-sm max-w-md mx-auto" style={{ color: 'var(--theme-text-muted)' }}>
            {projects.length === 0
              ? t('webhooks.create_project_first')
              : t('webhooks.choose_project')}
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-x-2 rounded-xl bg-apple-blue px-4 py-2.5 text-sm font-medium text-white shadow hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-apple-blue focus:ring-offset-2 transition-colors"
          >
            <PlusIcon className="-ml-0.5 h-5 w-5" aria-hidden="true" />
            {t('webhooks.add_endpoint')}
          </button>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center" style={{ color: 'var(--theme-text-muted)' }}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {webhook.events.map((e: string) => (
                        <span key={e} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium" style={{ backgroundColor: 'var(--theme-bg-input)', color: 'var(--theme-text-secondary)' }}>
                          {e}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-3">
                      <button 
                        ...
                        className="text-apple-blue hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors"
                      >
                      ...
                      <button 
                        ...
                        className={`transition-colors ${selectedEndpointId === webhook.id ? 'text-green-600 dark:text-green-400' : 'text-apple-gray-400 hover:text-apple-gray-600 dark:hover:text-gray-300'}`}
                      >
                      ...
                      <button 
                        ...
                        className="text-apple-gray-400 hover:text-apple-gray-600 dark:hover:text-gray-300 transition-colors"
                      >
          ...
          >
            <div className="px-6 py-4 border-b flex justify-between items-center" style={{ borderColor: 'var(--theme-border)', backgroundColor: 'var(--theme-bg-input)' }}>
              ...
              <button 
                ...
                className="transition-colors" style={{ color: 'var(--theme-text-muted)' }}
              >
                {t('common.close')}
              </button>
            </div>
            ...
            >
              <div className="px-6 py-5 border-b" style={{ borderColor: 'var(--theme-border)' }}>
                <h3 className="text-xl font-semibold" style={{ color: 'var(--theme-text)' }}>
                  {editingWebhook ? t('webhooks.edit_webhook') : t('webhooks.add_webhook')}
                </h3>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-3 p-3 rounded-lg cursor-pointer hover:opacity-80 transition-colors" style={{ backgroundColor: 'var(--theme-bg-input)', border: '1px solid var(--theme-border)' }}>
                      <input 
                        type="checkbox" 
                        className="rounded border-gray-300 text-apple-blue focus:ring-apple-blue cursor-pointer h-4 w-4 bg-transparent"
                      ...
                      />
                      <span className="text-sm font-medium" style={{ color: 'var(--theme-text)' }}>ping</span>
                      <span className="text-xs ml-auto" style={{ color: 'var(--theme-text-muted)' }}>{t('webhooks.ping_desc')}</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 rounded-lg cursor-pointer hover:opacity-80 transition-colors" style={{ backgroundColor: 'var(--theme-bg-input)', border: '1px solid var(--theme-border)' }}>
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className="text-sm font-medium" style={{ color: 'var(--theme-text)' }}>payment.succeeded</span>
                      <span className="text-xs ml-auto" style={{ color: 'var(--theme-text-muted)' }}>{t('webhooks.payment_desc')}</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <div className="flex h-6 items-center">
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-apple-blue focus:ring-apple-blue bg-transparent cursor-pointer"
                    />
                  </div>
                ...
                <div className="pt-4 flex items-center justify-end gap-3 border-t mt-6" style={{ borderColor: 'var(--theme-border)' }}>
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="px-4 py-2.5 rounded-xl text-sm font-medium transition-colors hover:opacity-80"
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2.5 rounded-xl text-sm font-medium text-white shadow bg-apple-blue hover:bg-blue-600 transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-apple-blue"
                  >
