  ...
  if (error) {
    return (
      <div className="p-4 bg-red-50 text-apple-red rounded-apple border border-red-100">
        {t('sla.load_error', { message: error.message })}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-apple-gray-900">{t('sla.title')}</h1>
          <p className="text-apple-gray-500 mt-1">{t('sla.subtitle')}</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          >
            <option value={1}>{t('sla.window_1h')}</option>
            <option value={24}>{t('sla.window_24h')}</option>
            <option value={168}>{t('sla.window_7d')}</option>
            <option value={720}>{t('sla.window_30d')}</option>
          </select>
          <button
            onClick={() => refetch()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm font-medium text-apple-gray-500">{t('sla.platform_throughput')}</p>
          </div>
          <p className="text-3xl font-semibold text-apple-gray-900 mt-4">
            {sla?.totalRequests.toLocaleString() ?? 0}
          </p>
          <p className="text-xs text-apple-gray-400 mt-1">{t('sla.requests_in_window')}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm font-medium text-apple-gray-500">{t('sla.error_rate')}</p>
          </div>
          <p className="text-3xl font-semibold text-apple-gray-900 mt-4">
            {((sla?.failureRate ?? 0) * 100).toFixed(2)}%
          </p>
          <p className="text-xs text-apple-gray-400 mt-1">
            {t('sla.error_rate_desc')}
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card md:col-span-2 lg:col-span-2">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-sm font-medium text-apple-gray-500">{t('sla.request_latency_ms')}</p>
          </div>
          <div className="grid grid-cols-3 gap-4 mt-2 border-t border-apple-gray-100 pt-4">
            <div>
              <p className="text-xs text-apple-gray-400 uppercase tracking-wider mb-1">{t('sla.average')}</p>
            ...
            <div>
              <p className="text-xs text-apple-gray-400 uppercase tracking-wider mb-1">{t('sla.p95')}</p>
            ...
            <div>
              <p className="text-xs text-apple-gray-400 uppercase tracking-wider mb-1">{t('sla.p99')}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card md:col-span-2 lg:col-span-4">
           <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium text-apple-gray-500">{t('sla.upstream_availability')}</p>
            </div>
          </div>
          <div className="bg-apple-gray-50 p-4 rounded-apple border border-apple-gray-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-apple-gray-500 mt-1">{t('sla.active_providers_passing')}</p>
            </div>
            <div className="text-right">
              {sla?.activeProviders === 0 ? (
                <span className="text-sm text-apple-gray-400">{t('sla.no_active_providers')}</span>
              ) : sla?.healthyProviders === sla?.activeProviders ? (
                <span className="inline-flex items-center rounded-full bg-green-50 px-2.5 py-1 text-sm font-medium text-apple-green ring-1 ring-inset ring-green-600/20">{t('sla.all_healthy')}</span>
              ) : (
                <span className="inline-flex items-center rounded-full bg-red-50 px-2.5 py-1 text-sm font-medium text-apple-red ring-1 ring-inset ring-red-600/10">{t('sla.degraded')}</span>
              )}
            </div>
          </div>
        </motion.div>
